...
const onCopyConfigClick = () => {
  let copyThemeConfig: any = Local.get('themeConfig')
...
onMounted(() => {
  nextTick(() => {
    ...
    const frequency: any = Local.get('frequency')
    if (!frequency) initLayoutChangeFun()
    ...
    setTimeout(() => {
      ...
      if (themeConfig) locale.value = themeConfig.globalI18n
      // 初始化菜单样式等
      initSetStyle()
    }, 100)
  })

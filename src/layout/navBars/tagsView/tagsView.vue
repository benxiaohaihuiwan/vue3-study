  ...
  >
    <el-scrollbar ref="scrollbarRef" @wheel.prevent="onHandleScroll">
      ...
      >
        ...
        >
          <i
            class="iconfont icon-webicon318 layout-navbars-tags-view-ul-li-iconfont"
            v-if="isActive(v)"
          ></i>
          <SvgIcon
            :name="v.meta.icon"
          ...
          <template v-if="isActive(v)">
            <SvgIcon
              ...
              class="ml5 layout-navbars-tags-view-ul-li-refresh"
...
// pinia 中获取路由信息，如果设置了固定的（isAffix），进行初始化
...
const solveAddTagsView = async (path: string, to?: RouteToFrom) => {
  ...
  let current = state.tagsViewList.filter((v: RouteItem) => {
    const vParams = to?.meta?.isDynamic
      ? v.params
        ? v.params
        : null
      : v.query
      ? v.query
      : null
    const toParams = to?.meta?.isDynamic
      ? to.params
        ? to.params
        : null
      : to.query
      ? to.query
      : null
    v.path === isDynamicPath && isObjectValueEqual(vParams, toParams)
  })
  if (!current.length) {
...
const addTagsView = (path: string, to?: RouteToFrom) => {
  nextTick(async () => {
    ...
    if (to?.meta?.isDynamic) {
      // 动态路由（xxx/:id/:name）： 参数不同，开启多个tagsview
      if (!getThemeConfig.value.isShareTagsView) {
...
.layout-navbars-tags-view {
  background-color: var(--el-color-white);
  border-bottom: 1px solid var(--next-border-color-light);
  position: relative;
  z-index: 4;
  :deep(.el-scrollbar__warp) {
